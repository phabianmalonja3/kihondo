      ...
      <section ref={containerRef} className="py-24 px-6 bg-[#FCFDFD] overflow-hidden">
        <div className="max-w-7xl mx-auto">
          ...
          >
              ...
              {events.map((event, index) => (
                  ...
                  >
                    <Link href={`/blog/${event.id}`} className="block group">
                      {/* Changed bg-slate-900 to bg-slate-950 to fix white wash */}
                      <div className="event-card relative h-[500px] w-full rounded-[2.5rem] overflow-hidden shadow-2xl bg-slate-950 transition-all duration-500">
                        ...
                        <div className="relative z-10 h-full p-8 flex flex-col justify-end">
                          <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                            <div className="w-12 h-1 bg-emerald-400 mb-6 rounded-full group-hover:w-20 transition-all" />
                            <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-emerald-400">
                              {event.title}
                            </h3>
                            <p className="text-slate-200/80 text-sm line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity leading-relaxed">
                              {event.description}
                            </p>
                          </div>
                          <div className="absolute top-8 right-8 w-12 h-12 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white scale-0 group-hover:scale-100 transition-transform">
            ...
            {/* NAVIGATION: Buttons and Progress side-by-side at the bottom */}
            <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-8">
              {/* Progress bar pagination (Left) */}
              <div className="flex items-center gap-4">
                <span className="text-xs font-mono text-slate-400">0{current + 1}</span>
                <div className="h-[2px] w-48 bg-slate-100 relative overflow-hidden rounded-full">
                  <div
              ...
              {/* Combined Navigation Buttons (Right) */}
              <div className="flex items-center gap-3">
                <CarouselPrevious className="static translate-y-0 h-14 w-14 border-slate-200 text-slate-500 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all" />
                <CarouselNext className="static translate-y-0 h-14 w-14 border-slate-200 text-slate-500 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all" />
              </div>
            </div>
          </Carousel>
        </div>
