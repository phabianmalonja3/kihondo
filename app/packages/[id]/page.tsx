  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        // 2. Fetch All Packages to filter "Related" (Simplest way)
        const relatedRes = await fetch(`${baseUrl}/packages`);
        const relatedData = await relatedRes.json();
        const all = relatedData.packages?.data ?? relatedData;


// 1. Keep it as a string (UUIDs are strings)
const currentId = String(id); 

// 2. Filter: Compare strings to strings
const filtered = all.filter((item: Package) => {
  // Use .toString() just in case the API returns it differently
  // Compare UUIDs directly as strings
  const isDifferentPackage = item.id.toString() !== currentId;
  
  // Safety check: Make sure location exists before checking .name
  // Also comparing by name (String)
...
setRelated(filtered);
  ...
  const optionsArray = typeof pkg.options === 'string' ? JSON.parse(pkg.options) : pkg.options;

  return (
    <div className="min-h-screen bg-white">
      {/* Navigation & Hero */}
      <div className="relative h-[60vh] w-full">
        <Image src={pkg.image_url} alt={pkg.name} fill className="object-cover" priority unoptimized />
        <div className="absolute inset-0 bg-black/30" />
        <Link href="/packages" className="absolute top-6 left-6 flex items-center gap-2 text-white bg-black/20 backdrop-blur-md px-4 py-2 rounded-full hover:bg-black/40 transition">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-emerald-50 rounded-3xl p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Array.isArray(optionsArray) && optionsArray.map((opt, i) => (
    ...
    <div className="max-w-7xl mx-auto px-6 py-20 animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-10">
        <div className="col-span-2 space-y-6">
          ...
          <div className="h-20 bg-gray-200 w-full rounded-lg" />
        </div>
